...
Window {
    ...
    required property AudioExporter exporter
    ...
    Binding {
        dialog.exporter.config: AudioPreference.audioExporterEnableAdvancedOptions ? AudioExporterPresets.currentConfig : dialog.addOn.simpleConfig
    }
    ...
    Dialog {
        ...
        onAboutToShow: () => {
            ...
            if (config.fileType !== 0) {
                ...
                const estimatedBytes = msec / 1000 * config.formatSampleRate * (config.formatMono ? 1 : 2) * (4 - config.formatOption)
            ...
            ColumnLayout {
                ...
                GridLayout {
                    ...
                    RowLayout {
                        Layout.fillWidth: true
                        TextField {
                            id: simplePathTextField
                            Layout.fillWidth: true
                            readOnly: true
                            text: dialog.addOn.calculateSimplePath(dialog.addOn.simpleConfig)
                        }
                        Button {
                            text: qsTr("&Browse...")
                            icon.source: "image://fluent-system-icons/folder_open"
                            onClicked: dialog.addOn.browseFileSimple()
                        }
                    }
                    Label {
                        text: qsTr("Format")
                    ...
                    ComboBox {
                        id: simpleFormatComboBox
                        Layout.fillWidth: true
                        model: [
                            { text: qsTr("WAV"), value: 0 },
                            { text: qsTr("Ogg Vorbis"), value: 2 },
                        ]
                        textRole: "text"
                        valueRole: "value"
                        Component.onCompleted: () => {
                            currentIndex = Qt.binding(() => indexOfValue(dialog.addOn.simpleConfig.fileType))
                        }
                        onActivated: (index) => {
                            dialog.addOn.setFileTypeSimple(valueAt(index))
                        }
                    }
                    Label {
                    ...
                    ComboBox {
                        id: simpleSampleRateComboBox
                        Layout.fillWidth: true
                        model: [
                            { text: qsTr("%L1 Hz").arg(48000), value: 48000 },
                            { text: qsTr("%L1 Hz").arg(44100), value: 44100 },
                        ]
                        textRole: "text"
                        valueRole: "value"
                        Component.onCompleted: () => {
                            currentIndex = Qt.binding(() => indexOfValue(dialog.addOn.simpleConfig.formatSampleRate))
                        }
                        onActivated: (index) => {
                            dialog.addOn.simpleConfig.formatSampleRate = valueAt(index)
                        }
                    }
                    CheckBox {
                        id: simpleMonoCheckBox
                        Layout.columnSpan: 2
                        text: qsTr("Export using mono")
                        checked: dialog.addOn.simpleConfig.formatMono
                        onClicked: dialog.addOn.simpleConfig.formatMono = checked
                    }
                    CheckBox {
                        id: simpleSeparateCheckBox
                        Layout.columnSpan: 2
                        text: qsTr("Separate tracks")
                        checked: dialog.addOn.simpleConfig.mixingOption === 2
                        onClicked: dialog.addOn.setMixingOptionSimple(checked ? 2 : 0)
                    }
                }
                ColumnLayout {
                    id: advancedOptionsLayout
                    spacing: 8
                    visible: AudioPreference.audioExporterEnableAdvancedOptions
                    GroupBox {
                        id: presetGrouopBox
