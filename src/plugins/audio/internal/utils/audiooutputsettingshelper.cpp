...
namespace Audio::Internal {
    ...
    AudioOutputSettingsHelper::AudioOutputSettingsHelper(QObject *parent)
        ...
        connect(outputContext, &talcs::AbstractOutputContext::deviceChanged, 
                this, &AudioOutputSettingsHelper::onDeviceChanged);
        connect(outputContext, &talcs::AbstractOutputContext::bufferSizeChanged, 
                this, &AudioOutputSettingsHelper::onBufferSizeChanged);
        connect(outputContext, &talcs::AbstractOutputContext::sampleRateChanged, 
                this, &AudioOutputSettingsHelper::onSampleRateChanged);
    ...
    void AudioOutputSettingsHelper::updateDeviceList() {
        ...
        if (!outputContext->device()) {
            ...
            if (!currentDeviceFound) {
                ...
                QString displayName = currentDeviceName.isEmpty() ? 
                    tr("Default device") + " " + tr("(Not working)") :
                    currentDeviceName + " " + tr("(Not working)");
    ...
    void AudioOutputSettingsHelper::findDeviceIndex() {
        ...
        if (outputContext && outputContext->device()) {
            ...
            if (newIndex == -1) {
                QString notWorkingDisplayName = currentDeviceName.isEmpty() ? 
                    tr("Default device") + " " + tr("(Not working)") :
                    currentDeviceName + " " + tr("(Not working)");
    ...
    void AudioOutputSettingsHelper::setDeviceCurrentIndex(int index) {
        ...
        if (!m_outputSystem->setDevice(deviceName)) {
            // Setting failed, emit error signal
            QString errorMessage = deviceName.isEmpty() ? 
                tr("Audio device %1 is not available").arg(tr("Default device")) :
                tr("Audio device %1 is not available").arg(deviceName);
