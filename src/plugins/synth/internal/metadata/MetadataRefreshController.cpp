    ...
    namespace {
        ...
        QString apiErrorText(const Api::ApiError &error) {
            ...
            if (error.isResponseError())
                return Synth::Internal::MetadataRefreshController::tr("The synthesis service returned HTTP status %1").arg(error.httpStatusCode);
            return Synth::Internal::MetadataRefreshController::tr("The synthesis service request failed: network error %1").arg(error.networkErrorCode);
        }
        ...
        void request(const std::shared_ptr<State> &state, bool includeMetadata) {
            ...
            watch(state, generation, apiClient->getInfo(state->configuration),
                ...
                if (!result || result->hasError() || !negotiatedVersion) {
                    QString message;
                    if (!result)
                        message = Synth::Internal::MetadataRefreshController::tr("The health check was canceled");
                    else if (result->hasError())
                        message = apiErrorText(result->error());
                    else
                        message = Synth::Internal::MetadataRefreshController::tr("The service does not support DSSP API version 1");
        ...
        void finishMetadataLists(const std::shared_ptr<State> &state, quint64 generation) {
            ...
            if (!state->architecturesResult || state->architecturesResult->hasError()) {
                errors.append(state->architecturesResult
                                  ...
                                  : Synth::Internal::MetadataRefreshController::tr("The architecture request was canceled"));
            }
            if (!state->singersResult || state->singersResult->hasError()) {
                errors.append(state->singersResult
                                  ...
                                  : Synth::Internal::MetadataRefreshController::tr("The singer request was canceled"));
            }
            if (!errors.isEmpty()) {
            ...
            for (qsizetype index = 0; index < state->refreshedSingers.size(); ++index) {
                ...
                watch(state, generation,
                    ...
                    if (!stopping && !state->removed && generation == state->generation && result &&
                        ...
                        state->assetErrors.append(Synth::Internal::MetadataRefreshController::tr("Could not refresh a singer avatar"));
                    }
                ...
                watch(state, generation,
                    ...
                    if (!stopping && !state->removed && generation == state->generation && result &&
                        ...
                        state->assetErrors.append(Synth::Internal::MetadataRefreshController::tr("Could not refresh a singer background"));
                    }
                    finishAsset(state, generation);
                });
                watch(state, generation,
                    ...
                    if (!stopping && !state->removed && generation == state->generation && result &&
                        ...
                        state->assetErrors.append(Synth::Internal::MetadataRefreshController::tr("Could not refresh singer demo audio"));
                    }
                    finishAsset(state, generation);
                });
            }
