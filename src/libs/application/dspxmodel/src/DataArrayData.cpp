#include "DataArrayData_p.h"

#include <QJSEngine>

namespace dspx {

    QJSValue DataArrayJSIterable::create(QObject *o) {
        auto engine = qjsEngine(o);
        if (!engine) {
            qFatal() << "iterable() can only be called from QML";
            return {};
        }
        auto listIterableConstructor = engine->importModule(":/qt/qml/DiffScope/DspxModel/qml/iterable.mjs").property("DataArrayIterable");
